...
export class CalendarModal {
    ...
    switch (this._d.pickMode) {
      ...
      case "range":
        if (this._d.defaultDateRange) {
          if (this._d.defaultDateRange.from) {
            this.datesTemp[0] = this.calSvc.createCalendarDay(
              this._getDayTime(this._d.defaultDateRange.from),
              this._d
            )
          }
          if (this._d.defaultDateRange.to) {
            this.datesTemp[1] = this.calSvc.createCalendarDay(
              this._getDayTime(this._d.defaultDateRange.to),
              this._d
            )
          }
        }
      ...
      case "multi":
        if (this._d.defaultDates && this._d.defaultDates.length) {
          this.datesTemp = this._d.defaultDates.map(e =>
  ...
  createYearPicker(startTime: number, endTime: number) {
    ...
    this.options.range_beg =
      firstDayOfYear.getTime() < startTime
        ? startTime
        : firstDayOfYear.getTime()
  ...
  backwardsMonth() {
    ...
    let firstTime = moment(first.original.time)
      .subtract(1, "M")
      .valueOf()
  ...
  scrollToDefaultDate() {
    ...
    let defaultDateMonth = this.monthsEle.nativeElement.children[
  ...
  changedYearSelection() {
    ...
    // checking whether the start is after firstDayOfYear
    this.options.range_beg =
      firstDayOfYear.getTime() < this.options.start
        ? this.options.start
        : firstDayOfYear.getTime()
    // checking whether the end is before lastDayOfYear
    this.options.range_end =
      lastDayOfYear.getTime() > this.options.end
        ? this.options.end
